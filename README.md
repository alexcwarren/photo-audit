...
Read-only auditing and comparison of large photo and video collections.
...
This project requires Python 3.12 or newer and pins its normal development interpreter in `.python-version`.
...
```shell
git clone https://github.com/alexcwarren/photo-audit.git
cd photo-audit
uv sync
```
...
```shell
uv run python -m photo_audit.main
```
...
```shell
uv run pytest --cov=photo_audit --cov-report=term-missing
```
